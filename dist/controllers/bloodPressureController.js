...
const createBloodPressure = (req, res) => {
  ...
  if (!systolic || !diastolic || !date || !time) {
    return res.status(400).json({
      error: "Faltan campos obligatorios del registro de presion arterial."
    });
  }
  ...
  writeData(data);
  res.json({
    message: "Registro cargado con éxito",
    blood: newBlood
  });
};
const updateBloodPressure = (req, res) => {
  ...
  if (systolic) {
    ...
    if (!systolicValidation.isValid) {
      return res.status(400).json({
        error: systolicValidation.error
      });
    }
  ...
  if (diastolic) {
    ...
    if (!diastolicValidation.isValid) {
      return res.status(400).json({
        error: diastolicValidation.error
      });
    }
  ...
  writeData(data);
  res.json({
    message: "Datos de presión arterial actualizados con éxito",
    blood
  });
};
...
export { getBloodPressure, getBloodPressureByUsername, createBloodPressure, updateBloodPressure, deleteBlood };